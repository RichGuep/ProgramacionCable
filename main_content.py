...
def run_app():
    ...
    if df_users is None: df_users = pd.DataFrame(columns=["Nombre", "Correo", "Rol", "Password"])
    ...
    df_raw = load_base()
    with st.sidebar:
        menu = st.radio("Menú", ["🏠 Inicio", "📊 Gestión de Mallas", "⚙️ Usuarios"])
    ...
    if menu == "📊 Gestión de Mallas":
        ...
        if df_hist is None: df_hist = pd.DataFrame(columns=["Mes", "Año", "Tipo", "Fecha", "Datos_JSON"])

        t1, t2 = st.tabs(["⚡ Generador", "📜 Histórico"])
        ...
        with t1:
            ...
            if st.button("GENERAR PROGRAMACIÓN"):
                # Lógica de Empalme: Buscar mes pasado
                ...
                if not pasado.empty:
                    try:
                        ...
                        st.info(f"🔄 Continuidad aplicada desde {m_ant}")
                    except: pass
            ...
            if 'm_temp' in st.session_state:
                ...
                if existe:
                    st.warning(f"⚠️ Ya existe una malla para {mes_sel}. ¿Deseas reemplazarla?")
                    confirmar = st.checkbox("SÍ, ESTOY SEGURO DE REEMPLAZAR")
                else: confirmar = True
                ...
                if st.button("💾 GUARDAR EN HISTÓRICO"):
                    if confirmar:
                        ...
                        if guardar_excel_en_github(df_hist, "historico_mallas.xlsx"):
                            st.success("✅ ¡Malla sincronizada con GitHub!"); st.balloons()
                    else: st.error("Debes marcar la casilla de confirmación.")

        with t2:
            if not df_hist.empty:
                sel = st.selectbox("Versión", range(len(df_hist)), format_func=lambda x: f"{df_hist.iloc[x]['Mes']} {df_hist.iloc[x]['Año']}")
                if st.button("Visualizar Seleccionada"):
                    try:
                    ...
                    except: st.error("Error al leer histórico")
        ...
        with st.form("UserForm"):
            ...
            if st.form_submit_button("Registrar"):
                df_users = pd.concat([df_users, pd.DataFrame([[n,c,r,p]], columns=df_users.columns)], ignore_index=True)
                if guardar_excel_en_github(df_users, "usuarios.xlsx"): st.success("OK"); st.rerun()
