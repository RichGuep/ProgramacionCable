...
def run_app():
    ...
    # 2. Cargar Datos desde SQLite (Usuarios y Personal)
    df_users = read_db("usuarios")
    if df_users is None:
        # Usuario Admin por defecto si la base está vacía
    ...
    if df_raw is None:
        df_raw = load_base() # Respaldo: Carga el excel si no hay tabla SQL
    ...
    if not st.session_state['auth']:
        ...
        with col_login:
            ...
            st.markdown('<div class="brand-subtitle">Gestión Operativa SQLite</div>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        # Solo mostramos el periodo si estamos trabajando en mallas
    ...
    if st.session_state['menu_actual'] == "🏠 Inicio":
        ...
        st.write("Seleccione un módulo de gestión:")
        st.divider()
        
        c1, c2, c3 = st.columns(3)
        with c1:
            st.markdown('<p style="color:#059669; font-weight:bold; font-size:18px;">OPERACIONES</p>', unsafe_allow_html=True)
        ...
        with c2:
            st.markdown('<p style="color:#2563eb; font-weight:bold; font-size:18px;">RECURSOS</p>', unsafe_allow_html=True)
        ...
        with c3:
            st.markdown('<p style="color:#d97706; font-weight:bold; font-size:18px;">CONFIGURACIÓN</p>', unsafe_allow_html=True)
            if st.button("⚙️ USUARIOS\n\nRoles y Seguridad", use_container_width=True):
    ...
    # --- VISTA: GESTIÓN DE MALLAS ---
        ...
        tab1, tab2, tab3 = st.tabs(["⚙️ Parametrización", "⚡ Vista y Reportes", "📜 Histórico"])
        ...
        with tab2:
            if 'temp_malla_tec' in st.session_state:
                ...
                with f_row[0]:
                    ...
                    v_ini = r_c1.date_input("Desde", f_inicio, key="v_ini")
                    v_fin = r_c2.date_input("Hasta", f_fin, key="v_fin")
                ...
                if st.button("💾 GUARDAR VERSIÓN EN SQLITE", use_container_width=True):
                    ...
                    if save_db(pd.concat([df_hist, nueva], ignore_index=True), "historico_mallas"):
                        st.success("✅ Guardado y Sincronizado."); st.balloons()
    ...
    # --- VISTA: BASE DE DATOS (GESTIÓN DE PERSONAL) ---
        ...
        st.header("👥 Gestión de Personal (SQLite)")
            ...
            with st.form("f_reg", clear_on_submit=True):
                ...
                if st.form_submit_button("💾 GUARDAR EN BASE DE DATOS"):
                    if nom:
                        ...
                        if save_db(df_new, "empleados"): 
                            st.success(f"{nom.upper()} registrado."); st.rerun()
        ...
        if df_raw is not None and not df_raw.empty:
            ...
            if st.button("❌ ELIMINAR") and elim:
                if save_db(df_raw[df_raw['nombre'] != elim], "empleados"): 
                    st.warning("Eliminado de la base."); st.rerun()
