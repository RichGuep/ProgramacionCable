...
def login_page():
    ...
    if not st.session_state['auth']:
        ...
        with col_login:
            ...
            # Logos alineados abajo (Logo 3 al centro)
...
if df_raw is not None:
    ...
    if st.button("🚀 GENERAR MALLA ÓPTIMA"):
        ...
        for _, row in df_f.iterrows():
            ...
            # BAJAMOS LA META: Para dar espacio real a los compensatorios
        ...
        if LpStatus[prob.status] == 'Optimal':
            ...
            for emp, grupo in df_res.groupby("Empleado"):
                ...
                # 1. Primero marcamos los DESC. CONTRATO (Los 2 de ley)
                idx_f = grupo[(grupo['Turno'] == '---') & (grupo['Nom_Dia'] == d_c_n)].head(2).index
                grupo.loc[idx_f, 'Turno'] = 'DESC. CONTRATO'
                
                # 2. IDENTIFICACIÓN DE COMPENSATORIOS (Lunes a Viernes)
                # Por cada fin de semana donde TRABAJÓ (no es DESC. CONTRATO ni DISPO), damos un compensatorio
                findes_laborados = grupo[(grupo['Nom_Dia'] == d_c_n) & (grupo['Turno'].isin(LISTA_TURNOS))]
                
                for _, row_f in findes_laborados.iterrows():
                    dia_t = row_f['Dia']
                    # Buscamos hueco libre en los siguientes 7 días Lunes a Viernes
                    hueco = grupo[(grupo['Dia'] > dia_t) & (grupo['Dia'] <= dia_t + 7) & (grupo['Turno'] == '---') & (~grupo['Nom_Dia'].isin(['Sab', 'Dom']))].head(1)
                    if not hueco.empty:
                        grupo.loc[hueco.index, 'Turno'] = 'DESC. L-V'
                
                # 3. Solo al final lo que sobre es DISPO
    ...
    if 'df_final' in st.session_state:
        ...
        t1, t2, t3 = st.tabs(["📅 Malla Operativa", "🔍 Filtro Empleado", "⚖️ Auditoría"])
        ...
        with t3:
            ...
            for e, g in df_v.groupby("Empleado"):
                ...
                audit.append({"Empleado": e, "Findes Trabajados": f_t, "Compensatorios L-V": len(g[g['Turno'] == 'DESC. L-V']), "Ley 1-1": "✅ Cumple" if len(g[g['Turno'] == 'DESC. L-V']) >= f_t else "⚠️ Revisar"})
