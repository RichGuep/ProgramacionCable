...
if df_raw is not None:
    ...
    if st.button("🚀 GENERAR MALLA DE RELEVOS"):
        ...
        for _, row in df_f.iterrows():
            ...
            # Solo 2 descansos de Ley al mes
            ...
            for s_idx, dias_s in enumerate(semanas):
                ...
                if dia_cont_sem:
                    d_c = dia_cont_sem[0]
                    # Solo genera compensatorio si trabajó su día de ley
        ...
        if LpStatus[prob.status] in ['Optimal', 'Not Solved']:
            ...
            # Clasificación Inteligente: Titular vs Disponible
            for d in range(1, num_dias + 1):
                for t in ["AM", "PM", "Noche"]:
                    indices = df_res[(df_res['Dia'] == d) & (df_res['Turno'] == t)].index
            ...
            st.error("🚨 La lógica de relevos falló. Revisa la cantidad de personal.")
    ...
    if 'df_final' in st.session_state:
        ...
        def style_richard(v):
            ...
            if 'COMP' in v: return 'background-color: #ffd966; color: black'
            return ''

        st.dataframe(m_f[cols].style.map(style_richard), use_container_width=True)
