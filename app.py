...
# --- 1. CONFIGURACIÓN ---
st.set_page_config(page_title="MovilGo - Regla de Oro con Excepción", layout="wide", page_icon="🚫")
...
if df_raw is not None:
    ...
    num_g = 4 
    st.title(f"🛡️ Malla: Relevo T3 Autorizado")
    ...
    if st.button("⚡ GENERAR MALLA (BLOQUEO T3 + RELEVO)"):
        g_rotan = [g for g in g_finales if t_map[g] == "ROTA"]
        prob = LpProblem("MovilGo_Final_Safe", LpMinimize)
        ...
        for s in semanas:
            prob += lpSum([asig[g][s]["T3"] for g in g_rotan]) == 1 # Exclusividad T3 semanal
            prob += lpSum([asig[g][s]["T1"] for g in g_rotan]) >= 1
            prob += lpSum([asig[g][s]["T2"] for g in g_rotan]) >= 1
            for g in g_rotan:
        ...
        final_rows = []
        turno_vivo = {g: res_semanal.get((g, semanas[0]), "DISPONIBLE") for g in g_rotan}
        ...
        for d_i in d_info:
            ...
            hoy_labels = {}
            relevo_t3_necesario = None
            
            for g in g_rotan:
                if dia_nom == d_map[g]:
                    # Si el que descansa estaba en T3, marcamos que necesitamos relevo T3
                    if turno_vivo[g] == "T3": relevo_t3_necesario = g
                    hoy_labels[g] = "DESC. LEY"
                    turno_vivo[g] = res_semanal.get((g, d_i["sem"]), "DISPONIBLE")
                else:
                    hoy_labels[g] = turno_vivo[g]
            
            # Lógica Disponibilidad con RELEVO T3
            if dia_nom == d_map[g_disp]:
                label_disp = "DESC. LEY"
            elif relevo_t3_necesario:
                label_disp = "T3" # EXCEPCIÓN: Disponible entra a T3 porque el titular descansa
            else:
                label_disp = "T1" # Base
                for g_off in descansan_hoy:
                    label_disp = turno_vivo[g_off]
                    break

            for g in g_finales:
                res_f = label_disp if g == g_disp else hoy_labels[g]
                for _, m in df_celulas[df_celulas['grupo'] == g].iterrows():
                    final_rows.append({"Dia": d_i["n"], "Label": d_i["label"], "Empleado": m['nombre'], "Cargo": m['cargo'], "Grupo": g, "Final": res_f})
